...
//        List<Task> trainTasks = controller.getTasks(true);
        ...
        final List<T2<Task.Sample, Double>> metrics = new ArrayList<>();
...
//                metaMinima += Math.log(horzM.getA() * vertM.getA() * vertM.getB() * vertM.getA());
        ...
        for(T2<Task.Sample, Double> p : metrics)
        {
            VBox pair = new VBox(5);
            ColorGrid inGrid = p.getA().input;
            ColorGrid outGrid = p.getA().output;

            pair.getChildren().add(new Label(String.format("%d, %3.3f, %d x %d → %d x %d", controller.getIndex(p.getA().task.getTaskCode()),
                                                           p.getB(),                                                           inGrid.getWidth(), inGrid.getHeight(), outGrid.getWidth(), outGrid.getHeight())));
...
//            float[][] indiff = pattern.autodiff(outGrid.toFloat());
